#######################################################
####### Filter and clean the genome collection ########
#######################################################
## Code: Ariel Favier (afavier@uci.edu)
## Last updated: Aug 19 2026
#######################################################

# This is a script to filter the genome collection obtained from 
# SPADES based on the genome completeness and contamination score
# after applying CheckM software to each assembly
# Genome assembly and taxonomy from KBase narrative: 
# https://doi.org/10.25982/200311.171/3014786
...
paste(parent,"02_TCS_genome_collection/", sep="") %>% setwd

#### Reading CheckM output file ####
genome_quality <- fread("02_Data/TCS_genome_quality.csv")

# We assign quality scores as specified in Table 3 of Parks et al. 2015 (DOI: 10.1101/gr.186072.114). 
# Given that the genomes were obtained from isolate populations,
# their completeness levels are all very high (Near Completeness),
...
taxonomy_file_kbase <- fread("/02_Data/taxonomy_file_kbase.csv")
...
write.csv(Kbase_taxonomy_quality_unfiltered,"03_Output/Kbase_taxonomy_quality_unfiltered.csv",row.names = FALSE)

write.csv(Kbase_taxonomy_quality_filtered,"03_Output/Kbase_taxonomy_quality_filtered.csv",row.names = FALSE)
...
write.csv(quast_stats_filtered, "03_Output/quast_stats_filtered.csv", row.names = FALSE)
